...
class AsyncHttpClient:
    def __init__(self, config: F1Config):
        ...
        self._request_semaphore = asyncio.Semaphore(max(1, config.max_concurrent_requests))
        self.client = AsyncClient(
            base_url=config.base_url,
            timeout=config.timeout,
        )

    async def _maybe_wait_before_request(self) -> None:
        async with self._rate_limit_lock:
            if (
                self.config.rate_limit_enabled
                and self.config.pause_every_requests > 0
                and self.config.pause_seconds > 0
                and self._request_count > 0
                and self._request_count % self.config.pause_every_requests == 0
            ):
                LOGGER.debug(
                    "Async rate-limit pause before request (request_count=%d, pause_seconds=%s).",
                    self._request_count,
                    self.config.pause_seconds,
                )
    ...
    async def _send_get(
        ...
        async with self._request_semaphore:
            await self._maybe_wait_before_request()
            has_auth = "Authorization" in headers
            LOGGER.debug("Async HTTP GET %s (auth=%s, params=%d).", path, has_auth, len(query))
            response = await self.client.get(path, params=query, headers=headers or None)
            LOGGER.debug("Async HTTP GET %s -> %d", path, response.status_code)
            return response

    async def _send_get_with_retries(
        self,
        path: str,
        query: dict[str, str],
        headers: dict[str, str],
        ...
        for attempt in range(1, attempts + 1):
            response = await self._send_get(path=path, query=query, headers=headers)
            ...
            if retry_after:
                ...
                delay = self.config.retry_backoff_seconds * attempt
            LOGGER.warning(
                "Async HTTP GET %s returned %d, retrying in %.2fs (attempt=%d/%d).",
                path,
    ...
    async def get_list(self, path: str, params: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        q = build_query(params)

        headers = self._build_auth_headers(force_refresh=False)
        r = await self._send_get_with_retries(path=path, query=q, headers=headers)

        if r.status_code == 401 and self.config.token_provider is not None:
            LOGGER.warning("Async HTTP GET %s returned 401, retrying with refreshed token.", path)
            refreshed_headers = self._build_auth_headers(force_refresh=True)
            if refreshed_headers:
                r = await self._send_get_with_retries(path=path, query=q, headers=refreshed_headers)

        try:
            r.raise_for_status()
        except HTTPStatusError as exc:
            LOGGER.error("Async HTTP GET %s failed with status %d.", path, r.status_code)
            raise OpenF1HTTPError(str(exc), r.status_code, r.request, r) from exc

        data = r.json()
        if not isinstance(data, list):
            raise OpenF1HTTPError(f"Expected list JSON, got {type(data)}", r.status_code, r.request, r)
        LOGGER.debug("Async HTTP GET %s returned %d rows.", path, len(data))
        return data
